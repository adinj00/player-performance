...
public sealed class Venue : Entity
{
    ...
    private Venue(Guid id, string name, string normalizedName, DateTime utcNow) : base(id)
    {
        Name = name;
    ...
    public static Venue Create(Guid id, string name, string normalizedName, DateTime utcNow) => new(id, name, normalizedName, utcNow);
    ...
    {
        if (IsArchived)
            throw new InvalidOperationException("Archived venues cannot be updated.");
    }
}
