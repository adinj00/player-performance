...
internal sealed class IdentityStaffUsersService(AppDbContext db, UserManager<ApplicationUser> users, ISystemClock clock) : IStaffUsersService
{
    ...
    public async Task<IReadOnlyList<StaffUserResponse>> ListAsync(string? search, StaffRole? role, string? status, TeamScopeType? scopeType, Guid? teamId, CancellationToken ct)
    {
        ...
        if (!string.IsNullOrWhiteSpace(search))
        {
            ...
            query = query.Where(x => x.p.DisplayName.ToUpper().Contains(value) || (x.u.Email ?? "").ToUpper().Contains(value));
        }
        if (role.HasValue)
            query = query.Where(x => x.p.PrimaryRole == role.Value);
        if (Enum.TryParse<UserAccountStatus>(status, true, out var parsed))
            query = query.Where(x => x.u.AccountStatus == parsed);
        if (scopeType.HasValue)
            query = query.Where(x => x.p.TeamScopeType == scopeType.Value);
        if (teamId.HasValue)
            query = query.Where(x => db.StaffTeamScopes.Any(s => s.UserId == x.p.UserId && s.TeamId == teamId));
    ...
    {
        if (!TryAccess(request.DisplayName, request.PrimaryRole, request.CanVerifyReports, request.CanImportData, request.CanViewMedicalDetails, request.TeamScopeType, request.SelectedTeamIds, out var access))
            return Result<StaffInvitationCredentialResponse>.Failure(StaffUserErrors.Validation);
        var email = request.Email?.Trim();
        if (string.IsNullOrWhiteSpace(email) || !new EmailAddressAttribute().IsValid(email))
            return Result<StaffInvitationCredentialResponse>.Failure(StaffUserErrors.Validation);
        if (await users.FindByEmailAsync(email) is not null)
            return Result<StaffInvitationCredentialResponse>.Failure(StaffUserErrors.DuplicateEmail);
        if (!await ValidTeamsAsync(access.Scope, access.TeamIds, ct))
            return Result<StaffInvitationCredentialResponse>.Failure(StaffUserErrors.Validation);
    ...
    public async Task<Result<StaffUserResponse>> UpdateProfileAsync(Guid userId, UpdateStaffProfileRequest request, CancellationToken ct)
    {
        if (string.IsNullOrWhiteSpace(request.DisplayName) || request.DisplayName.Trim().Length > 120)
            return Result<StaffUserResponse>.Failure(StaffUserErrors.Validation);
    ...
    {
        if (!TryAccess("valid", request.PrimaryRole, request.CanVerifyReports, request.CanImportData, request.CanViewMedicalDetails, request.TeamScopeType, request.SelectedTeamIds, out var access) || !await ValidTeamsAsync(access.Scope, access.TeamIds, ct))
            return Result<StaffUserResponse>.Failure(StaffUserErrors.Validation);
        var profile = await db.StaffAccessProfiles.SingleOrDefaultAsync(x => x.UserId == userId, ct);
        ...
        profile.CanViewMedicalDetails = access.Medical;
        profile.TeamScopeType = access.Scope;
        profile.UpdatedUtc = clock.UtcNow;
    ...
    public async Task<Result<StaffUserResponse>> DisableAsync(Guid userId, CancellationToken ct)
    {
        ...
        if (user.AccountStatus == UserAccountStatus.DISABLED)
            return Result<StaffUserResponse>.Success((await GetAsync(userId, ct))!);
        if (await IsFinalActiveAdminAsync(userId, ct))
            return Result<StaffUserResponse>.Failure(StaffUserErrors.Conflict);
    ...
    {
        if (string.IsNullOrWhiteSpace(request.Email) || string.IsNullOrWhiteSpace(request.Token) || string.IsNullOrWhiteSpace(request.Password) || request.Password != request.ConfirmPassword)
    ...
    private void AddScopes(Guid userId, IReadOnlyList<Guid> ids, DateTimeOffset now)
    {
        foreach (var id in ids)
            db.StaffTeamScopes.Add(new StaffTeamScope { UserId = userId, TeamId = id, CreatedUtc = now });
    }
    private static bool TryAccess(string? name, StaffRole role, bool? verify, bool? import, bool? medical, TeamScopeType scope, IReadOnlyList<Guid>? ids, out (string Name, StaffRole Role, bool Verify, bool Import, bool Medical, TeamScopeType Scope, IReadOnlyList<Guid> TeamIds) access)
    {
        access = default;
        if (string.IsNullOrWhiteSpace(name) || name.Trim().Length > 120 || !Enum.IsDefined(role) || !Enum.IsDefined(scope) || !verify.HasValue || !import.HasValue || !medical.HasValue || ids is null)
            return false;
        var admin = role == StaffRole.ADMIN;
        if ((admin || scope == TeamScopeType.ALL_TEAMS) && ids.Count != 0)
            return false;
        if (!admin && scope == TeamScopeType.SELECTED_TEAMS && ids.Count == 0)
            return false;
        access = (name.Trim(), role, admin || verify.Value, admin || import.Value, admin || medical.Value, admin ? TeamScopeType.ALL_TEAMS : scope, admin || scope == TeamScopeType.ALL_TEAMS ? [] : ids);
        return true;
    }
    ...
    private async Task<IReadOnlyList<StaffUserResponse>> MapAsync(IEnumerable<(StaffAccessProfile p, ApplicationUser u)> rows, CancellationToken ct)
    {
        var list = rows.ToList();
        var ids = list.Select(x => x.p.UserId).ToList();
        var scopes = await db.StaffTeamScopes.AsNoTracking().Where(x => ids.Contains(x.UserId)).OrderBy(x => x.TeamId).ToListAsync(ct);
