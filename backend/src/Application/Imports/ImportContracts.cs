...
public sealed record CreateImportRequest(Guid TeamId, Guid? MatchId, Guid? TrainingSessionId, ImportType ImportType, ImportSourceSystem SourceSystem, string? SourceLabel, string? Description, string OriginalFileName, string? ContentType, long? DeclaredLength, Stream Content);
public sealed record ImportListQuery(Guid? TeamId = null, Guid? MatchId = null, Guid? TrainingSessionId = null, ImportType? ImportType = null, ImportSourceSystem? SourceSystem = null, ImportFileFormat? FileFormat = null, ImportJobStatus? Status = null, string? Search = null, DateTime? DateFrom = null, DateTime? DateTo = null, int Page = 1, int PageSize = 25);
public sealed record ImportJobResponse(Guid Id, Guid TeamId, Guid? MatchId, Guid? TrainingSessionId, ImportType ImportType, ImportSourceSystem SourceSystem, string? SourceLabel, ImportFileFormat FileFormat, ImportJobStatus Status, string? Description, string OriginalFileName, string ContentType, long SizeBytes, DateTime CreatedAtUtc, DateTime UpdatedAtUtc, int ConfigurationRevision, int? ValidatedConfigurationRevision, DateTime? ValidatedAtUtc, DateTime? PreviewGeneratedAtUtc, DateTime? ValidationCompletedAtUtc, int? TotalRowCount, int? PreviewRowCount, int? ValidRowCount, int? InvalidRowCount, int? WarningCount, string? FailureCode, string? FailureMessage, string? PreviewMetadataJson, DateTime? ConfirmedAtUtc, DateTime? CancelledAtUtc, IReadOnlyList<ImportAllowedAction> AllowedActions);
