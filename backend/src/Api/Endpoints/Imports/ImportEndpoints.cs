...
internal static class ImportEndpoints
{
    ...
    private static async Task<IResult> ListAsync(IImportService s, HttpContext h, CancellationToken ct, Guid? teamId = null, Guid? matchId = null, Guid? trainingSessionId = null, ImportType? importType = null, ImportSourceSystem? sourceSystem = null, ImportFileFormat? fileFormat = null, ImportJobStatus? status = null, string? search = null, DateTime? dateFrom = null, DateTime? dateTo = null, int page = 1, int pageSize = 25) => ToResult(await s.ListAsync(new(teamId, matchId, trainingSessionId, importType, sourceSystem, fileFormat, status, search, dateFrom, dateTo, page, pageSize), ct), h);
