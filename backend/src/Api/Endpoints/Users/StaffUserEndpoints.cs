...
internal static class StaffUserEndpoints
{
    ...
    private static async Task<IResult> CreateAsync(HttpContext c, IAntiforgery a, CreateStaffInvitationRequest r, IStaffUsersService s, CancellationToken ct)
    {
        ...
        return f ?? ToCreated(await s.CreateInvitationAsync(r, ct), c);
    }
    private static async Task<IResult> ReissueAsync(Guid userId, HttpContext c, IAntiforgery a, IStaffUsersService s, CancellationToken ct)
    {
        var f = await AntiforgeryValidation.ValidateRequestAsync(c, a);
        return f ?? ToResult(await s.ReissueInvitationAsync(userId, ct), c);
    }
    private static async Task<IResult> UpdateAsync(Guid userId, HttpContext c, IAntiforgery a, UpdateStaffProfileRequest r, IStaffUsersService s, CancellationToken ct)
    {
        var f = await AntiforgeryValidation.ValidateRequestAsync(c, a);
        return f ?? ToResult(await s.UpdateProfileAsync(userId, r, ct), c);
    }
    private static async Task<IResult> ReplaceAccessAsync(Guid userId, HttpContext c, IAntiforgery a, ReplaceStaffAccessRequest r, IStaffUsersService s, CancellationToken ct)
    {
        var f = await AntiforgeryValidation.ValidateRequestAsync(c, a);
        return f ?? ToResult(await s.ReplaceAccessAsync(userId, r, ct), c);
    }
    private static async Task<IResult> DisableAsync(Guid userId, HttpContext c, IAntiforgery a, IStaffUsersService s, CancellationToken ct)
    {
        var f = await AntiforgeryValidation.ValidateRequestAsync(c, a);
        return f ?? ToResult(await s.DisableAsync(userId, ct), c);
    }
    private static async Task<IResult> ReactivateAsync(Guid userId, HttpContext c, IAntiforgery a, IStaffUsersService s, CancellationToken ct)
    {
        var f = await AntiforgeryValidation.ValidateRequestAsync(c, a);
        return f ?? ToResult(await s.ReactivateAsync(userId, ct), c);
    }
    private static async Task<IResult> AcceptAsync(HttpContext c, AcceptStaffInvitationRequest r, IStaffUsersService s, CancellationToken ct)
    {
        var result = await s.AcceptInvitationAsync(r, ct);
        return result.IsSuccess ? TypedResults.NoContent() : ToProblem(result.Error, c);
    }
    ...
    private static IResult ToProblem(Error e, HttpContext c)
    {
        var status = e.Code switch
        {
            "not_found" => 404,
            "duplicate_email" or "staff_conflict" => 409,
            "invalid_invitation" => 400,
            "validation_failed" => 422,
            _ => 400
        };
        return TypedResults.Problem(new ProblemDetails { Status = status, Title = "Request could not be completed", Detail = e.Message, Extensions = { ["code"] = e.Code, ["traceId"] = c.TraceIdentifier } });
    }
}
