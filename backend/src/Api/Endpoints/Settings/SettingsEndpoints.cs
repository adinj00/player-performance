...
internal static class SettingsEndpoints
{
    ...
    { var failure = await AntiforgeryValidation.ValidateRequestAsync(context, antiforgery); if (failure is not null) return failure; var result = await service.CreateSeasonAsync(request, ct); return result.IsSuccess ? TypedResults.Created($"/api/settings/seasons/{result.Value.Id}", result.Value) : ToProblem(result.Error, context); }
    ...
    { var failure = await AntiforgeryValidation.ValidateRequestAsync(context, antiforgery); if (failure is not null) return failure; return ToResult(await service.UpdateSeasonAsync(id, request, ct), context); }
    ...
    { var failure = await AntiforgeryValidation.ValidateRequestAsync(context, antiforgery); if (failure is not null) return failure; return ToResult(await service.ArchiveSeasonAsync(id, ct), context); }
    ...
    { var failure = await AntiforgeryValidation.ValidateRequestAsync(context, antiforgery); if (failure is not null) return failure; return ToResult(await service.RestoreSeasonAsync(id, ct), context); }
    ...
    { var failure = await AntiforgeryValidation.ValidateRequestAsync(context, antiforgery); if (failure is not null) return failure; var result = await service.CreateCompetitionAsync(request, ct); return result.IsSuccess ? TypedResults.Created($"/api/settings/competitions/{result.Value.Id}", result.Value) : ToProblem(result.Error, context); }
    ...
    { var failure = await AntiforgeryValidation.ValidateRequestAsync(context, antiforgery); if (failure is not null) return failure; return ToResult(await service.UpdateCompetitionAsync(id, request, ct), context); }
    ...
    { var failure = await AntiforgeryValidation.ValidateRequestAsync(context, antiforgery); if (failure is not null) return failure; return ToResult(await service.ArchiveCompetitionAsync(id, ct), context); }
    ...
    { var failure = await AntiforgeryValidation.ValidateRequestAsync(context, antiforgery); if (failure is not null) return failure; return ToResult(await service.RestoreCompetitionAsync(id, ct), context); }
    ...
    {
        var status = error.Code switch { "not_found" => StatusCodes.Status404NotFound, "duplicate_name" or "archived_record" => StatusCodes.Status409Conflict, "validation_failed" => StatusCodes.Status422UnprocessableEntity, _ => StatusCodes.Status400BadRequest };
