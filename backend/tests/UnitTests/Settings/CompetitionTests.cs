...
public sealed class CompetitionTests
{
    ...
    public void ArchiveAndRestore_ShouldBeIdempotent()
    {
        ...
        competition.Archive(DateTime.UtcNow); competition.Archive(DateTime.UtcNow); Assert.True(competition.IsArchived);
        competition.Restore(DateTime.UtcNow); competition.Restore(DateTime.UtcNow); Assert.False(competition.IsArchived);
    }
}
