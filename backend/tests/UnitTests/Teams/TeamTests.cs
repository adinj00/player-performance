...
public sealed class TeamTests
{
    ...
    public void Create_ShouldTrimNameAndSetInitialState()
    {
        ...
        Assert.Equal("U19", team.Name); Assert.Equal(TeamStatus.ACTIVE, team.Status); Assert.Equal(0, team.DisplayOrder); Assert.Equal(now, team.CreatedAtUtc); Assert.Equal(now, team.UpdatedAtUtc);
    }
    ...
    public void Lifecycle_ShouldFollowRequiredTransitionsAndBeIdempotent()
    {
        ...
        team.Deactivate(DateTime.UtcNow); team.Deactivate(DateTime.UtcNow); Assert.Equal(TeamStatus.INACTIVE, team.Status);
        team.Archive(DateTime.UtcNow); team.Archive(DateTime.UtcNow); Assert.Equal(TeamStatus.ARCHIVED, team.Status);
        team.Restore(DateTime.UtcNow); team.Restore(DateTime.UtcNow); Assert.Equal(TeamStatus.INACTIVE, team.Status);
        team.Activate(DateTime.UtcNow); Assert.Equal(TeamStatus.ACTIVE, team.Status);
    }
