...
This file is a reference catalog of shadcn/ui components available for consideration during frontend feature work.

It is not a requirement to install or use every component.

Before building a custom UI primitive or interaction pattern, check this catalog and prefer an existing shadcn/ui component when it matches the required behavior.

Install components just in time, only when the active feature spec requires them.

The official shadcn/ui documentation remains the source of truth for current component APIs and installation instructions.
