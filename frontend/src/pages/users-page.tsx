...
export function UsersPage() {
  ...
  const setFilters = useCallback(
    (updates: Partial<Record<keyof StaffUrlFilters, string | null>>) => {
  ...
  useEffect(() => {
    const timeoutId = window.setTimeout(() => {
      void setFiltersRef.current({ q: searchValue || null });
    }, 300);
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <section className="border-border bg-card flex flex-nowrap items-center gap-3 overflow-x-auto rounded-xl border p-4">
        ...
        <FilterSelect
          label="Uloge"
          clearLabel="Odaberite ulogu"
          value={filters.role}
          mapping={roleLabels}
          onChange={(role) => void setFilters({ role })}
        />
        <FilterSelect
          label="Status"
          clearLabel="Odaberite status"
          value={filters.status}
          mapping={statusLabels}
          onChange={(status) => void setFilters({ status })}
        />
        <FilterSelect
          label="Pristup timovima"
          clearLabel="Odaberite pristup timovima"
          value={filters.scope}
          mapping={scopeLabels}
          onChange={(scope) => void setFilters({ scope })}
        />
      </section>
      {staff.isLoading ? (
        <LoadingState />
    ...
    <div className="shrink-0">
      <Select
        ...
        onValueChange={(nextValue) =>
          onChange(nextValue === "__clear__" ? null : nextValue || null)
        }
      >
        <SelectTrigger aria-label={label} className="w-40">
          <SelectValue>{value ? mapping[value] : label}</SelectValue>
        </SelectTrigger>
        <SelectContent>
          <SelectGroup>
            <SelectLabel>{label}</SelectLabel>
            <SelectItem value="__clear__">{clearLabel}</SelectItem>
            {Object.entries(mapping).map(([key, text]) => (
